...
const props = withDefaults(defineProps<GradientTextProps>(), {
  text: '',
  className: '',
  colors: () => ['#0070f3', '#9c40ff', '#0070f3'], // 主题色：蓝色到紫色
  animationSpeed: 8,
  showBorder: false
});
...
<template>
  <div
    :class="`relative flex max-w-fit flex-row items-center justify-center rounded-[1.25rem] font-medium backdrop-blur transition-shadow duration-500 overflow-hidden cursor-pointer ${className}`"
  >
    <div
      v-if="showBorder"
    ...
    >
      <div
        class="absolute inset-0 rounded-[1.25rem] z-[-1]"
        style="width: calc(100% - 2px); height: calc(100% - 2px); left: 50%; top: 50%; transform: translate(-50%, -50%); background-color: var(--color-bg-dark);"
      />
    </div>
